...
int parse_instr(int line, char* ftok, instr* imem, int memoff, label_loc* labels, source* src) {
	...
	if ( pscnt > 0 ) {
		...
		switch( op ) {
			...
			//instruction added
			case MIN: case ROL: case BEXT: case SH3ADD: case XNOR:
			// case MUL:
            ...
            //BINVI,  // binvi rd, rs1, imm
            //SH1ADD, // sh1add rd, rs1, rs2
			case CLMULH: case MAX: case ORN: case BCLR: case SH1ADD:
			     if ( !o1 || !o2 || !o3 || o4 ) print_syntax_error( line,  "Invalid format" );
			 	    i->a1.reg = parse_reg(o1 , line);
			 	    i->a2.reg = parse_reg(o2 , line);
			 	    i->a3.reg = parse_reg(o3 , line);
			...
			case LB: case LBU: case LH: case LHU: case LW: case SB: case SH: case SW:
				...
				parse_mem(o2, &i->a2.reg, &i->a3.imm, 12, line);
				return 1;
			case ADDI: case SLTI: case SLTIU: case ANDI: case ORI: case XORI: case SLLI: case SRLI: case SRAI: case BEXTI:
				if ( !o1 || !o2 || !o3 || o4 ) print_syntax_error( line, "Invalid format" );
				i->a1.reg = parse_reg(o1, line);
				i->a2.reg = parse_reg(o2, line);
				i->a3.imm = signextend(parse_imm(o3, 12, line), 12);
			...
			case BEQ: case BGE: case BGEU: case BLT: case BLTU: case BNE:
				...
				return 1;
            case CPOP: case SEXT_B: case ZEXT_H:
                if ( !o1 || !o2 || o3 || o4 ) print_syntax_error( line, "Invalid format" );
                i->a1.reg = parse_reg(o1, line);
				i->a2.reg = parse_reg(o2, line);
                return 1;
	...
	while(!dexit) {
		...
		switch (i.op) {
			...
			//instruction added
      		...
      		case CLZ:
				if (rf[i.a2.reg] == 0) { rf[i.a1.reg] = 32; break;}
				{uint32_t x = rf[i.a2.reg], n;
				...
				rf[i.a1.reg] = n; break;}
			case MIN: rf[i.a1.reg] = (rf[i.a2.reg] < rf[i.a3.reg])? rf[i.a2.reg] : rf[i.a3.reg]; break;
			case ROL: rf[i.a1.reg] = (rf[i.a2.reg] << (rf[i.a3.reg] & 0x0000001F)) | rf[i.a2.reg] >> (32 - (rf[i.a3.reg] & 0x0000001F)); break;
			case BEXT: rf[i.a1.reg] = (rf[i.a2.reg] >> (rf[i.a3.reg] & 0x0000001F)) & 1; break;
			case BSETI: rf[i.a1.reg] = rf[i.a2.reg] | (1 << (i.a3.imm & 0x0000001F)); break;
			case SH3ADD: rf[i.a1.reg] =  (rf[i.a3.reg] + (rf[i.a2.reg] << 3)); break;
			...
			case MAXU:					//	maxu rd, rs1, rs2
				rf[i.a1.reg]  = (rf[i.a2.reg] > rf[i.a3.reg] ) ? rf[i.a2.reg] : rf[i.a3.reg]; break;

			case REV8:					//	rev8 rd, rs
				for(int k = 0; k < 32; k+=8)
				{
					shamt = (rf[i.a2.reg] >> k) & 0xff;
					x |= shamt << (32 - 8 - k);
				}
				rf[i.a1.reg] = x;
				break;
			...
			case BSET:					//	bset rd, rs1, rs2
				index2 = rf[i.a3.reg] & (32 - 1);
				rf[i.a1.reg] = rf[i.a2.reg] | ( 1 << index2);
				break;
			...
			case SH2ADD:				//	sh2add rd, rs1, rs2
      		...
      		//case MUL: rf[i.a1.reg] = rf[i.a2.reg] * rf[i.a3.reg]; break;
      		//CLMULH, // clmulh rd, rs1, rs2    // Carry-less multiply (high-part)  //done
            //MAX,    // max rd, rs1, rs2       // Maximum (signed)                 //done
            //ORN,    // orn rd, rs1, rs2       // OR with inverted operand         //done
            //BCLR,   // bclr rd, rs1, rs2      // Single-Bit Clear (Register)      //done
            //BINVI,  // binvi rd, rs1, imm     // Single-Bit Invert (Immediate)    //done
            //SH1ADD, // sh1add rd, rs1, rs2    // Shift left by 1 and add          //done
            case CLMULH:
                rf[i.a1.reg] = 0;
                ...
                break;
            case MAX: rf[i.a1.reg] = (*(signed int*)&rf[i.a2.reg]) < (*(signed int*)&rf[i.a3.reg]) ? rf[i.a3.reg] : rf[i.a2.reg] ; break;
            case ORN: rf[i.a1.reg] = rf[i.a2.reg] | (~rf[i.a3.reg]); break;
            case BCLR: rf[i.a1.reg] = rf[i.a2.reg] & (~(1 << (rf[i.a3.reg] & (32-1)) )); break;
            case BINVI: rf[i.a1.reg] = rf[i.a2.reg] ^ ( 1 << (i.a3.imm & (32-1) ) ); break;
            case SH1ADD: rf[i.a1.reg] = ( rf[i.a2.reg] << 1 ) + rf[i.a3.reg]; break;

      		//*****************
			//bitwise instruction
			case CLMUL:  //Carry-less multiply (low-part), clmul rd, rs1, rs2
				rf[i.a1.reg] = 0;
				for(uint8_t j = 0; j <= 31; ++j) {
					//printf( "j is %d ||", j);
					//printf( "output is %x ||", rf[i.a1.reg]);
					rf[i.a1.reg] = ( (rf[i.a3.reg] >> j) & 1 ) ? ( rf[i.a1.reg] ^ (rf[i.a2.reg] << j) ) : (rf[i.a1.reg]);
					//printf( "determine is %d ||", ((rf[i.a3.reg] >> j) & 1));
					//((rf[i.a3.reg] >> j) & 1) ? printf( "output xor with %x ||", (rf[i.a2.reg] << j)) : printf("output doesn't change ! ||");
					//printf( "finally, output is %x || \n", rf[i.a1.reg]);
				}
				break;
			case CTZ:  //Count trailing zeros, ctz rd, rs
				rf[i.a1.reg] = 0;
				for ( uint8_t ctr = 0; ((rf[i.a2.reg] & ((uint32_t)1<<ctr) ) >> ctr ) == 0 ; ctr++) { rf[i.a1.reg]++; }
				break;
			case ORC_B:  //Bitwise OR-Combine, byte granule, orc.b rd, rs
				for(uint8_t p = 0; p <= 24; p = p + 8) {

					//printf("p is %d ||" , p);
					bool one_or_zero = ( ((rf[i.a2.reg] & (    ((uint32_t(1)<<(p+7)) - 1) + (uint32_t(1)<<(p+7))    ) )>> p) == 0 ) ? 0 : 1;
					//printf("mask is %x ||", (((uint32_t(1)<<(p+7)) - 1) + ((uint32_t(1)<<(p+7)))));
					//printf("determine is %d ||",((rf[i.a2.reg] & (    ((uint32_t(1)<<(p+7)) - 1) + (uint32_t(1)<<(p+7))    ) )>> p));
					//printf(" one_or_zero is %d ||", one_or_zero);
					rf[i.a1.reg] = !one_or_zero ? ( (rf[i.a1.reg] & (uint32_t)0x00ffffff)) : ( (rf[i.a1.reg] | (uint32_t)0xff000000));
					if( p < 24) rf[i.a1.reg] = rf[i.a1.reg] >> 8;
					//printf("rd is %x \n", rf[i.a1.reg]);
			...
			case RORI:  //Rotate Right (Immediate), rori rd, rs1, shamt
				...
				break;
			case BINV:{  //Single-Bit Invert (Register), binv rd, rs1, rs2
				uint8_t specific_bit_index = (rf[i.a3.reg] & (uint8_t)0b11111); //get the 5 lsbs of rs2
				rf[i.a1.reg] = rf[i.a2.reg] ^ ((uint32_t)1 << specific_bit_index);  //flip the rs1[specific_bit_index]
			}
				break;
			case SEXT_H:  //Sign-extend halfword, sext.h rd, rs
				rf[i.a1.reg] = signextend(rf[i.a2.reg], 16);
				break;
			//===================
            ...
            case ROR:
                {
                    int shamt = rf[i.a3.reg] & 0b11111;
                    rf[i.a1.reg] = ( rf[i.a2.reg] >> shamt ) | ( rf[i.a2.reg] << (32 - shamt) );
                }
                break;
            case BEXTI:
                {
                    int index = i.a3.imm & 0b11111;
                    rf[i.a1.reg] = ( rf[i.a2.reg] >> index ) & 0b1;
                }
                break;
            case SEXT_B:
                rf[i.a1.reg] = (int32_t)(int8_t)( rf[i.a2.reg] & 0xFF );
                break;
            case ZEXT_H:
                rf[i.a1.reg] = (uint16_t)( rf[i.a2.reg] & 0xFFFF );
                break;
